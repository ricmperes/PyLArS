...
def plot_rates()
    ...
    ax.bar(np.arange(25), 200/result_df['livetime'].dt.total_seconds(), alpha = 1, label = 'Double coin (2xPMT)')
    ax.bar(np.arange(25), result_df['rate'], alpha = 1, label = 'Triple coin (2xPMT + SiPM)')
    ...
    plt.savefig('coin_rates.jpeg', dpi = 120, bbox_inches='tight', pad_inches=0.1)
    plt.show()
...
def plot_baseline_all_runs():
    ...
    big_ax.set_xlabel('Number of dataset', labelpad=10, )  # set the common x label
    big_ax.set_ylabel('Baseline [ADCcounts]', labelpad=25)  # set the common y label

    plt.savefig('baselines_all_runs.jpeg', dpi = 120, bbox_inches='tight', pad_inches=0.1)
    plt.show()    
